...
func (c *SearchRole) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "systemconfiguration/role/searchRole/searchRole.html"
	...
	if err != nil {
		...
		if language == "english" {
			err = errors.New(beego.AppConfig.String("EN_ROLE_FETCH_FAILED"))
			log.Println(beego.AppConfig.String("loglevel"), "ENGLISH Error", err)
			return
		} else if language == "french" {
			err = errors.New(beego.AppConfig.String("FN_ROLE_FETCH_FAILED"))
...
func (c *SearchRole) Post() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.Data["DisplayMessage"] = " "
			c.TplName = "systemconfiguration/role/searchRole/searchRole.html"
			log.Println(beego.AppConfig.String("loglevel"), "Info", "Search Role  Page Success")
		}
		return
	}()
	...
	data, err := services.SearchRoleByFilter(input_name+"%", from, to, input_status)
	if err != nil {
		log.Println(beego.AppConfig.String("loglevel"), "Error", err)
		//err = errors.New("usertype fetch Failed")
		if language == "english" {
