...
func (c *ViewRole) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "systemconfiguration/role/viewRole/viewRole.html"
	...
	if err != nil {
		...
		if language == "english" {
			err = errors.New(beego.AppConfig.String("EN_UNABLE_TO_GET_ROLE_DATA"))
			log.Println(beego.AppConfig.String("loglevel"), "ENGLISH Error", err)
			return
		} else if language == "french" {
			err = errors.New(beego.AppConfig.String("FN_UNABLE_TO_GET_ROLE_DATA"))
	...
	if len(data) <= 0 {
		...
		if language == "english" {
			err = errors.New(beego.AppConfig.String("EN_ROLE_DATA_NOT_FOUND"))
			log.Println(beego.AppConfig.String("loglevel"), "ENGLISH Error", err)
			return
		} else if language == "french" {
			err = errors.New(beego.AppConfig.String("FN_ROLE_DATA_NOT_FOUND"))
			log.Println(beego.AppConfig.String("loglevel"), "FRENCH Error", err)
			return
		}
		return
	...
	//fmt.Println("menu in view page", menuarr)
	//fmt.Println("submenu in view page", submenuarr)
