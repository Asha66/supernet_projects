...
func (c *CreateuserType) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "systemconfiguration/usertype/createuserType/createuserType.html"
			log.Println(beego.AppConfig.String("loglevel"), "Info", "Create Usertype Page Fail")
		} else {
...
func (c *CreateuserType) Post() {
	...
	if err != nil {
		...
		if language == "english" {
			err = errors.New(beego.AppConfig.String("EN_USERTYPE_ALREADY_EXISTS"))
			log.Println(beego.AppConfig.String("loglevel"), "ENGLISH Error", err)
			return
		} else if language == "french" {
			err = errors.New(beego.AppConfig.String("FN_USERTYPE_ALREADY_EXISTS"))
			log.Println(beego.AppConfig.String("loglevel"), "FRENCH Error", err)
			return
		}
		return
	...
	result, err := db.Db.Exec(`INSERT INTO usertypes (id,
	    name,
		"desc",
		status,
		created_by,
	...
	if err != nil {
		...
		if language == "english" {
			err = errors.New(beego.AppConfig.String("EN_USERTYPE_CREATION_FAILED"))
			log.Println(beego.AppConfig.String("loglevel"), "ENGLISH Error", err)
			return
		} else if language == "french" {
			err = errors.New(beego.AppConfig.String("FN_USERTYPE_CREATION_FAILED"))
			log.Println(beego.AppConfig.String("loglevel"), "FRENCH Error", err)
			return
		}
		return
	...
	if language == "english" {
		...
		usertypemsg = beego.AppConfig.String("EN_USRTYPE_CREATESUCCESFULLY")

	} else if language == "french" {

		usertypemsg = beego.AppConfig.String("FN_USRTYPE_CREATESUCCESFULLY")
...
func CheckUserAlreadyExists(name string) (err error) {
	...
	row, err := db.Db.Query(`select count(*) from usertypes WHERE name = $1`, name)
