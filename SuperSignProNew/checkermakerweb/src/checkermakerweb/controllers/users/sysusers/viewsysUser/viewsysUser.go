...
func (c *ViewsysUser) Get() {
	...
	defer func() {
		...
		if err != nil {
			...
			c.TplName = "users/sysusers/viewsysUsers/viewsysUsers.html"
	...
	row, err := db.Db.Query(`select sysuser.id,sysuser.fullname,sysuser.mobile,sysuser.email,sysuser.address,sysuser.language,roles.role_name from sysuser
	...
	for i := range data {
		...
		c.Data["Language"] = data[i][5]
		c.Data["Rolename"] = data[i][6]
